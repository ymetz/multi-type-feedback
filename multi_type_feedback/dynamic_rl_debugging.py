...
class DynamicRLHFDebugging(DynamicRLHF):
    ...
    def train_iteration(self, sampling_strategy: str = "random"):
        ...
        for i, feedback_item in enumerate(feedback):
            for feedback_type, feedback_data in feedback_item.items():
                ...
                feedback_entry = {
                    ...
                    "trajectory_idx": i,
                    "trajectory_reward": (
                        trajectory_rewards[i] if i < len(trajectory_rewards) else None
                    ),
                    "data": feedback_data,
                }
        ...
        # Track buffer sizes
        buffer_sizes = {
            ft: len(self.feedback_buffers[ft]) for ft in self.feedback_types
        }
    ...
    def evaluate_rl_agent(self, n_episodes=5):
        ...
        # Track metrics
        self.debug_stats["rl_performance"]["episode_returns"].append(
            np.mean(episode_returns)
        )
        self.debug_stats["rl_performance"]["episode_lengths"].append(
    ...
    def compute_model_uncertainty(self, trajectory, feedback_type):
        ...
        self.debug_stats["uncertainties"][feedback_type].append(
            (len(self.debug_stats["iteration_metrics"]), uncertainty)
        )
    ...
    def compute_ensemble_reward(self, state, action):
        ...
        ):  # Sample 1% of rewards to avoid excessive memory usage
            self.debug_stats["ensemble_rewards"].append(
                {
                    "iteration": len(self.debug_stats["iteration_metrics"]),
                    "state": (
                        state[0].copy() if isinstance(state, np.ndarray) else state
                    ),
                    "action": (
                        action[0].copy() if isinstance(action, np.ndarray) else action
                    ),
    ...
    def sample_feedback_uncertainty(self, trajectories, initial_states):
        ...
        feedback, feedback_counts = super().sample_feedback_uncertainty(
            trajectories, initial_states
        )
        ...
        if len(self.debug_stats["iteration_metrics"]) > 0:
            ...
            for feedback_type, uncertainties in uncertainty_data.items():
                if uncertainties:
                    current_metrics[f"mean_uncertainty_{feedback_type}"] = np.mean(
                        uncertainties
                    )
                    current_metrics[f"max_uncertainty_{feedback_type}"] = np.max(
                        uncertainties
                    )
