...
def collect_results(
    ...
    if len(algorithms) > 0:
        for algorithm in algorithms:
            ...
            algorithm_df = process_algorithm_results(
                model_base_path, algorithm, full_param_df=full_param_df
            )
            all_results = pd.concat([all_results, algorithm_df], ignore_index=True)
    else:
        algorithm_df = process_algorithm_results(
            model_base_path, None, full_param_df=full_param_df
        )
        all_results = pd.concat([all_results, algorithm_df], ignore_index=True)
...
def process_algorithm_results(
    model_base_path: str, algorithm: str = None, full_param_df: bool = False
) -> pd.DataFrame:
        ...
        try:
            ...
            if not full_param_df:
                result_dict = {
                    "algorithm": (
                        the_dict["algorithm"]
                        if "algorithm" in the_dict
                        else the_dict["algo"]
                    ),
                    "environment": (
                        the_dict["environment"]
                        if "environment" in the_dict
                        else the_dict["env"]
                    ),
                    "run": run_dir,
                    "seed": the_dict["seed"],
...
def main():
    ...
    parser.add_argument(
        "--model-base-path", default="train_baselines/gt_agents"
