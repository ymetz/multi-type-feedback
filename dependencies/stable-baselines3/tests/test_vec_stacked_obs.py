...
def test_compute_stacking_box():
    ...
    channels_first, stack_dimension, stacked_shape, repeat_axis = compute_stacking(
        N_STACK, observation_space=space
    )
...
def test_reset_update_multidim_box_channel_first():
    ...
    stacked_observations = StackedObservations(
        NUM_ENVS, N_STACK, space, channels_order="first"
    )
    observations_1 = np.stack([space.sample() for _ in range(NUM_ENVS)])
    stacked_obs = stacked_observations.reset(observations_1)
...
def test_reset_update_image_channel_first_stack_last():
    space = spaces.Box(0, 255, (C, H, W), dtype=np.uint8)
    stacked_observations = StackedObservations(
        NUM_ENVS, N_STACK, space, channels_order="last"
    )
    observations_1 = np.stack([space.sample() for _ in range(NUM_ENVS)])
    stacked_obs = stacked_observations.reset(observations_1)
...
def test_reset_update_dict():
    space = spaces.Dict(
        {
            "key1": spaces.Box(0, 255, (H, W, C), dtype=np.uint8),
            "key2": spaces.Box(-1, 1, (4, 5)),
        }
    )
    stacked_observations = StackedObservations(
    ...
    observations_1 = {
        key: np.stack([subspace.sample() for _ in range(NUM_ENVS)])
...
def test_episode_termination_box():
    ...
    ] = terminal_observation  # episode termination in env1
    dones[1] = True
    ...
    true_stacked_obs_env1 = np.concatenate(
        (zeros, observations_1[0], observations_2[0], observations_3[0]), axis=-1
    )
    true_stacked_obs_env2 = np.concatenate(
        (zeros, zeros, zeros, observations_3[1]), axis=-1
    )
...
def test_episode_termination_dict():
    space = spaces.Dict(
        {
            "key1": spaces.Box(0, 255, (H, W, 3), dtype=np.uint8),
            "key2": spaces.Box(-1, 1, (4, 5)),
        }
    )
    stacked_observations = StackedObservations(
    ...
    for key, axis in zip(observations_1.keys(), [0, -1]):
        ...
        true_stacked_obs_env1 = np.concatenate(
            (
                zeros,
                observations_1[key][0],
                observations_2[key][0],
                observations_3[key][0],
            ),
            axis,
        )
        true_stacked_obs_env2 = np.concatenate(
            (zeros, zeros, zeros, observations_3[key][1]), axis
        )
        true_stacked_obs = np.stack((true_stacked_obs_env1, true_stacked_obs_env2))
