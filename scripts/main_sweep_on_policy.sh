...
seeds=(1687123)  # Reduced to 3 for faster initial sweep 1789 12 912391 330 1687123
...
create_job_script() {
    ...
    cat <<EOT > job_scripts_on_policy/job_${job_id}.sh
#!/bin/bash
        ...
        for feedback in "${feedback_types[@]}"; do
            for query_strategy in "${query_strategies[@]}"; do
                ...
                cmd="python multi_type_feedback/dynamic_rlhf.py \
                    ...
                    --wandb-project-name dynamic_rlhf_main_sweep"
...
for seed in "${seeds[@]}"; do
    for env in "${envs[@]}"; do
        # Use default hyperparameters for baseline
        job_name="ppo_baseline_${env}_s${seed}"
        
        cmd="python multi_type_feedback/train_RL_agent.py \
            --algorithm ppo \
            --environment $env \
            --feedback-type baseline \
            --seed $seed \
            --wandb-project-name dynamic_rlhf_main_sweep"
    ...
    for env in "${envs[@]}"; do
        for query_strategy in "${query_strategies[@]}"; do
            for reward_model_type in "${reward_model_types[@]}"; do 
                # Use default hyperparameters for baseline
                job_name="mtf-ppo_${env}_s${seed}_q${query_strategy}_r${reward_model_type}"
...
cat <<EOT > submit_jobs_on_policy.sh
#!/bin/bash
...
while [ \$job_id -lt \$TOTAL_JOBS ]; do
    ...
    if [ \$jobs_to_submit -gt 0 ]; then
        ...
        for ((i=0; i<jobs_to_submit && job_id<TOTAL_JOBS; i++)); do
            sbatch job_scripts_on_policy/job_\${job_id}.sh
...
chmod +x submit_jobs_on_policy.sh
...
echo "=================================="
echo "To submit jobs, run: ./submit_jobs_on_policy.sh"
