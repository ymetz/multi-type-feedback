...
# Phase 1: Single feedback type baseline experiments
echo "Phase 1: Creating single feedback type baseline jobs..."
for seed in "${seeds[@]}"; do
    for env in "${envs[@]}"; do
        for feedback in "${single_feedback[@]}"; do
            # Use default hyperparameters for baseline
            job_name="baseline_${env}_${feedback}_s${seed}"
...
# Phase 2: Multi-feedback experiments with different architectures
echo "Phase 2: Creating multi-feedback architecture comparison jobs..."
for seed in "${seeds[@]:0:2}"; do  # Use only first 2 seeds for this phase
    for env in "${envs[@]:0:2}"; do  # Focus on Swimmer and Walker2d
        for feedback_combo in "${combo_feedback[@]}"; do
            for model_type in "${reward_model_types[@]}"; do

                job_name="arch_${env}_${model_type}_s${seed}"
                job_name="${job_name}_$(echo $feedback_combo | sed 's/ /_/g')"
