...
for i in "${!envs[@]}"; do
  for j in "${!feedback_types[@]}"; do
      ...
      echo "Train Reward Models for ${envs[$i]} and FB Type ${feedback_types[$j]} with CUDA_VISIBLE_DEVICES=${cuda_devices[$j]}"
      python rlhf/train_reward_model_2.py --algorithm ppo --environment ${envs[$i]} --feedback-type ${feedback_types[$j]} &
  done
