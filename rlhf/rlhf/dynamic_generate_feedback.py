...
class SegmentBuffer:
    ...
    def get_segment(self, start: int, length: int) -> list:
        ...
        while insert_pos < len(self.done_indices) and self.done_indices[insert_pos] < end:
        ...
        segments = []
        segments.append(list(itertools.islice(self.buffer, start, relevant_done_indices[0])))
        
        for i in range(len(relevant_done_indices) - 1):
            segment = list(itertools.islice(self.buffer, 
                                          relevant_done_indices[i], 
                                          relevant_done_indices[i + 1]))
            segments.append(segment)
        
        segments.append(list(itertools.islice(self.buffer, 
                                            relevant_done_indices[-1], 
                                            end)))
...
class BalancedStreamingKMeans:
    ...
    def finalize(self):
        ...
        if len(X) > 0:
            ...
            representatives = cluster_sums[valid_clusters] / cluster_counts[valid_clusters, np.newaxis]
...
def generate_feedback_stream(
    ...
    for model_file in checkpoint_files:
        ...
        for step in range(steps_this_checkpoint):
            if model is not None:
                actions = model.predict(norm_env.normalize_obs(observation) if norm_env else observation, deterministic=True)[0]
            else:
            ...
            if step % segment_len == 0 and step > 0:
                ...
                if len(segment) >= segment_len // 2:  # Minimum length requirement
                    ...
                    segment_return = sum(s[2] * (gamma ** i) for i, s in enumerate(segment))
                    ...
                    for expert_model, exp_norm_env in expert_models:
                        ...
                        for t in range(segment_len):
                            action = expert_model.predict(exp_norm_env.normalize_obs(obs) if exp_norm_env else obs, deterministic=True)[0]
                            next_obs, rew, terminated, truncated, _ = environment.step(action)
                            current_return += rew * (gamma ** t)
                            current_demo.append((np.expand_dims(obs, axis=0), action, rew, terminated or truncated))
                    ...
                    if best_demo_return > segment_return:
                        best_demos_reservoir.add((best_demo, segment, best_demo_return - segment_return))
                    
                    # Add to clustering
                    obs = np.array([np.concatenate((s[0].squeeze(0).flatten(), 
    ...
    for i, (seg, ret) in enumerate(reservoir_segments):
        ...
        for demo, orig_seg, improvement in reservoir_demos:
            if str(orig_seg) == str(seg):
                ...
                break
        improvements.append((i, demo_improvement if demo_improvement is not None else 0))
    ...
    max_rating = 10
    ratings = max_rating - np.digitize(opt_gaps, np.linspace(min(opt_gaps), max(opt_gaps), max_rating))
    ...
    for _ in range(n_feedback):
        ...
        if abs(cluster_rewards[i] - cluster_rewards[j]) > tolerance:
            descr_preferences.append((i, j, 1) if cluster_rewards[i] < cluster_rewards[j] else (j, i, 1))
...
def load_expert_models(args, environment: gym.Env, checkpoints_path: str) -> List[Tuple[Union[PPO, SAC], VecNormalize]]:
    ...
    try:
        ...
        run_eval_scores = run_eval_scores.loc[run_eval_scores['env'] == args.environment]\
            .sort_values(by=['eval_score'], ascending=False)\
            .head(args.top_n_models)["run"].to_list()
        expert_model_paths = [path for path in expert_model_paths 
                            if path.split(os.path.sep)[-1] in run_eval_scores]
    except:
        print("[WARN] No eval benchmark results are available. Check your eval benchmarks")
