...
class DynamicRLHF:
    def __init__(
        ...
        env,
        algorithm: str = "ppo",
        feedback_types: List[str] = ["evaluative", "comparative", "demonstrative", "descriptive"],
        ...
        if enable_wandb:
            wandb.init(project="dynamic_rlhf", 
                      config={
                          "algorithm": algorithm,
        ...
        for feedback_type in self.feedback_types:
            if "ALE/" in self.env.spec.id or "procgen" in self.env.spec.id:
                model = LightningCnnNetwork(
                    ...
                    output_dim=1,
                    loss_function=calculate_single_reward_loss if feedback_type in ["evaluative", "descriptive"] 
    ...
    def sample_feedback_random(self, trajectories: List[List], initial_states: List[np.ndarray]) -> Dict:
    ...
    def sample_feedback_uncertainty(self, trajectories: List[List], initial_states: List[np.ndarray]) -> Dict:
        ...
        for trajectory, initial_state in zip(trajectories, initial_states):
            feedback = self.oracle.get_feedback(trajectory, initial_state, self.feedback_types)
            for feedback_type in self.feedback_types:
                uncertainties[feedback_type].append(feedback['uncertainty'][feedback_type])
        ...
        if total_uncertainty == 0:
            ...
            probs = [avg_uncertainties[ft] / total_uncertainty for ft in self.feedback_types]
    ...
    def compute_model_uncertainty(self, trajectory: List[Tuple[np.ndarray, np.ndarray, float, bool]], 
    ...
    def sample_feedback_uncertainty(self, trajectories: List[List], initial_states: List[np.ndarray]) -> Dict:
        ...
        for trajectory, initial_state, uncertainties in zip(trajectories, initial_states, trajectory_uncertainties):
            ...
            if total_uncertainty == float('inf'):
                ...
                untrained_types = [ft for ft in self.feedback_types 
            ...
            feedback = self.oracle.get_feedback(trajectory, initial_state, [feedback_type])
            feedback['selected_uncertainty'] = uncertainties[feedback_type]  # Store for logging
    ...
    def train_iteration(self, sampling_strategy: str = "random"):
        ...
        if sampling_strategy == "uncertainty":
            for f in feedback:
                if 'selected_uncertainty' in f:
                    mean_uncertainties['selected_uncertainty'].append(f['selected_uncertainty'])
        ...
        if self.enable_wandb:
            ...
            if mean_uncertainties:
                metrics.update({
                    "mean_selected_uncertainty": np.mean(mean_uncertainties['selected_uncertainty']),
                    "max_selected_uncertainty": np.max(mean_uncertainties['selected_uncertainty']),
                    "min_selected_uncertainty": np.min(mean_uncertainties['selected_uncertainty']),
                })
    ...
    def train_reward_models(self):
        ...
        for feedback_type in self.feedback_types:
            ...
            trainer.fit(self.reward_models[feedback_type], 
                       DataLoader(dataset, batch_size=32, shuffle=True))
            
            metrics[f"{feedback_type}_loss"] = trainer.callback_metrics["val_loss"].item()
    ...
    def compute_ensemble_reward(self, state: np.ndarray, action: np.ndarray) -> float:
        ...
        with torch.no_grad():
            for feedback_type, reward_model in self.reward_models.items():
                if len(self.feedback_buffers[feedback_type]) > 0:  # Only use models with feedback
                    if reward_model.ensemble_count > 1:
                        state_expanded = state_tensor.expand(reward_model.ensemble_count, *state_tensor.shape[1:])
                        action_expanded = action_tensor.expand(reward_model.ensemble_count, *action_tensor.shape[1:])
...
def main():
    ...
    parser.add_argument("--feedback-types", nargs="+", type=str, 
    ...
    parser.add_argument("--sampling-strategy", type=str, default="random",
                      choices=["random", "uncertainty"], help="Feedback sampling strategy")
