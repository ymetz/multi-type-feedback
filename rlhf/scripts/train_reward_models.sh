...
EOT
    ...
    for combination in "${batch[@]}"; do
        ...
        echo "python rlhf/train_reward_model.py --algorithm ppo --environment $env --feedback-type $feedback --n-feedback $n_feedback --seed $seed --noise-level $noise --no-loading-bar &" >> $sbatch_script
    done
